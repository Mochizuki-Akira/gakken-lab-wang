...
(function(){
  ...
  function setView(mode){
    ...
    body.classList.toggle('teacher', isTeacher);
    body.classList.toggle('teacher-mode', isTeacher);
    body.classList.toggle('student', !isTeacher);
    // 显式控制 teacher-only / student-only，防止各章样式不一致
    document.querySelectorAll('.teacher-only').forEach(e=>{
      e.style.display = isTeacher ? '' : 'none';
      e.hidden = !isTeacher;
    });
    document.querySelectorAll('.student-only').forEach(e=>{
      e.style.display = isTeacher ? 'none' : '';
      e.hidden = isTeacher;
    });
    const toggleBtn = document.getElementById('btn-teacher');
    if(toggleBtn){
      toggleBtn.setAttribute('aria-pressed', isTeacher ? 'true' : 'false');
    }
  }
